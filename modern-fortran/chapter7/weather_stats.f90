program weather_stats
   ...
   do i = 1, size(ids)
      ...
      max_wind(i) = maxval(wind_speed) !maxval is a stdlib func
      mean_wind(i) = mean(wind_speed)
   end do

   print *, 'Max wind speed:', maxval(wind_speed), ' at station:', ids(maxloc(max_wind))
   print *, 'Mean wind speed(high):', maxval(max_wind), ' at station:', ids(maxloc(mean_wind))
   print *, 'Mean wind speed(low):', minval(max_wind), ' at station:', ids(minloc(mean_wind))
